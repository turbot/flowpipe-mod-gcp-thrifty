locals {
  compute_disk_exceeding_max_size_query = <<-EOQ
  select
      project,
      zone,
      name as disk_name
    from
      gcp_compute_disk
    where
      size_gb > ${var.compute_disk_exceeding_max_size};
  EOQ
}
...
trigger "query" "detect_and_correct_compute_disks_exceeding_max_size" {
  ...
  sql      = local.compute_disk_exceeding_max_size_query
      ...
      actions = {
        "skip" = {
          ...
          pipeline_args = {
            ...
            text     = "Skipped Compute Disk ${param.disk_name} exceeding maximum size."
          }
