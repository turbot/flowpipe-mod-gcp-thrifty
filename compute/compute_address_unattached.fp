locals {
  compute_address_unattached_query = <<-EOQ
    select
...
trigger "query" "detect_and_correct_compute_addresses_if_unattached" {
  ...
  sql      = local.compute_address_unattached_query
...
pipeline "correct_compute_addresses_if_unattached" {
  ...
  step "transform" "items_by_id" {
    value = { for row in param.items : row.address_name => row }
  }
...
pipeline "correct_one_compute_address_if_unattached" {
  ...
  step "pipeline" "respond" {
    ...
    args = {
      ...
      notification_level = param.notification_level
      approvers          = param.approvers
      detect_msg         = "Detected Compute Address ${param.address_name} unattached."
      default_action     = param.default_action
      enabled_actions    = param.enabled_actions
      actions = {
        "skip" = {
          ...
          pipeline_args = {
            ...
            text     = "Skipped Compute Address ${param.address_name} unattached."
          }
          success_msg = "Skipped Compute Address ${param.address_name}."
          error_msg   = "Error skipping Compute Address ${param.address_name}."
        },
        "release" = {
          label        = "Release"
          value        = "release"
          ...
          success_msg = "Released Compute Address ${param.address_name}."
          error_msg   = "Error releasing Compute Address ${param.address_name}."
        }
      }
    }
  }
